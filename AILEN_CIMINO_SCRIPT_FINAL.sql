...
-- 4 Vista: libros_mas_vendidos_por_autor
-- Esta vista podría mostrar una lista de autores junto con los libros más vendidos de cada autor, ordenados por la cantidad total de unidades vendidas.
-- Sería útil para identificar qué libros de cada autor son los más populares entre los clientes y poder armar los flyer con remomendaciones y generarles un lugar llamativo dentro de la tienda. 
-- Tablas involucradas: AUTOR (a), LIBRO_AUTOR (la), LIBRO (l), VENTA (v).

CREATE VIEW libros_mas_vendidos_por_autor AS
SELECT a.ID_AUTOR, a.NOMBRE AS nombre_autor, l.ID_LIBRO, l.Titulo, COUNT(*) AS total_vendido
FROM AUTOR a
JOIN LIBRO_AUTOR la ON a.ID_AUTOR = la.ID_AUTOR
JOIN LIBRO l ON la.ID_LIBRO = l.ID_LIBRO
JOIN VENTA v ON l.ID_LIBRO = v.ID_LIBRO
...
CREATE VIEW ventas_por_editorial AS
SELECT e.ID_editorial, e.Nombre AS nombre_editorial, COUNT(*) AS total_ventas
FROM EDITORIAL e
JOIN LIBRO l ON e.ID_editorial = l.ID_editorial
JOIN VENTA v ON l.ID_LIBRO = v.ID_LIBRO
GROUP BY e.ID_editorial, e.Nombre;

select * from ventas_por_editorial
...
BEGIN
    ...
    SELECT Precio INTO total_precio
    FROM LIBRO
    WHERE ID_libro = libro_id;
    ...
    IF stock_actual >= cantidad THEN
        ...
        INSERT INTO VENTA (Fecha_venta, ID_cliente, ID_libro, Cantidad)
        VALUES (fecha_actual, cliente_id, libro_id, cantidad);
        
        -- Actualizar el inventario de libros disponibles
        UPDATE LIBRO
...
-- Esta tabla está diseñada para almacenar un registro de todos los cambios de precio realizados en los libros. Contiene los siguientes campos:
...
-- El trigger `registrar_cambio_precio` se activa después de que se actualiza un registro en la tabla `LIBRO`. Para cada fila actualizada, verifica si el precio anterior (`OLD.PRECIO`) es diferente al nuevo precio (`NEW.PRECIO`). Si hay un cambio de precio, registra los detalles del cambio, incluyendo el ID del libro, el precio anterior y el nuevo precio, en la tabla de auditoría `AuditoriaCambiosPrecio`.
-- Esto permite mantener un historial de todos los cambios de precios realizados en los libros, lo que proporciona una trazabilidad completa de las modificaciones de precios en la base de datos.
DELIMITER //
